...
const verifyGame = async (game) => {
  ...
  const igdbResponse = await axios.post(
    `https://api.igdb.com/v4/games/?fields=name,slug&limit=1&search=${game}&filter[category][eq]=0`,
    {
      fields: [],
  ...
  if (gameObject.length === 0) {
    const backupResponse = await axios.get(
      `https://www.backloggd.com/games/${sluggedName}/`
    );
    ...
    if (imageSrc.includes("co1pee")) {
      const backupResponse = await axios.get(
        `https://www.backloggd.com/games/${gameObject[0].slug}/`
      );
      const $ = cheerio.load(backupResponse.data);
      const image = $(".card-img");
      const imageSrc = image.attr("src");
...
const convertFormat = (game) => {
  ...
  gameName = gameName
    .replace(/[^\w\s-]/gi, "")
    .replace(/\s+/g, "-")
    .toLowerCase(); // Replace other special characters and spaces
  return gameName;
};
